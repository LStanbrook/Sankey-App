...
server <- function(input, output, session){
  ...
  # when loading data, update options in select main cols, update nodes colors
  # if cols with desired names in data, select them automatically
  # remove file input widget, add refresh button on top of data table
  ...
  observeEvent(input$data,
               {
                 ...
                 colors <- c("#228B22", "#FFEA00", "#FFAC1C", "#EE4B2B", "#00FFBF", "#FF7F50", "#4682b4",  "#583687",
                                "#acb839", "#6f71d9","#6fb24b","#c45cad","#46ca79","#c9407d","#46b57c","#d43f61","#36dee6","#c2562e","#43c29e","#d85750","#71b46c",
                                "#bc80d5","#406e21","#5e87d3","#d1972c","#7f265b","#99b257","#b7a850","#d8739f","#836723","#c8873a","#88212a","#da905e",
                                "#8d3820","#d56467")
  ...
  d <- reactive({
    ...
    # data2 <- data2[!grepl('^missing', data2$NAME, ignore.case = TRUE), ]
    # data2[!is.na(data2$PATHNO_ENCODED) & !is.na(data2$USUBJID_ENCODED), ]
  })
 ...
 # selectors for colour grouped nodes
  # observeEvent(input$node_unique, {
  #     if (isolate(input$node_unique)){
  #       insertUI(
  #                     selector = '#node_unique_selector',
  #                     where = 'afterEnd',
  #                     ui = checkboxInput('Colour for Group 1',
  #                                           label = 'Add a colour for group 1'
  #       )
  #     )
  #   } else {
  #     removeUI(
